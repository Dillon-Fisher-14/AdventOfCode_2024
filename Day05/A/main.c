...
int main(){
    ...
    // // test 
    // int index16 = pl_hm_lookup(&tCompleteRulesHashmap, 13);
    // ruleset_t ruleset16 = sbtCompleteRulesList[index16];

    // printf("Here are all the dependacies of 16: ");
    // for(int i = 0; i < pl_sb_size(ruleset16.depedencies); i++){
    //     printf("%d, ", ruleset16.depedencies[i]);
    // }
    // printf("\n");
        ...
        while(fgets(row, 128, file_id) != NULL) {
            ...
            for(int i = 0; i < updates_in_row; i++){
                ...
                for(int j = 0; j < i; j++){
                    ...
                    for(int k = 0; k < pl_sb_size(rulesetNum.depedencies); k++){
                        if(rulesetNum.depedencies[k] == nums[j])
                        {
                            pl_sb_del(rulesetNum.depedencies, k);
                        }
                    }
                    
                }

                if(pl_sb_size(rulesetNum.depedencies) != 0){
            ...
            if(allDependanciesFound){
                ...
                printf("All Dependancies Found, adding: %d\n", nums[updates_in_row/2]);
            } else {
